...
from django.views.decorators.csrf import csrf_exempt    # 수정 2209252128 좋아요 에러 제거
...
def post_update(request, post_id):
    if request.user.is_authenticated:
        ...
        if request.method == 'GET':
            ...
            form = UpdatePostForm(request.POST, request.FILES)  # 수정_220721231101
            if form.is_valid():
                post.caption = form.cleaned_data['caption']
                post.cooking_time = form.cleaned_data['cooking_time']   # 수정_2207192029
                post.image_01 = form.cleaned_data['image_01']    # 수정_2207072059
                post.image_02 = form.cleaned_data['image_02']    # 수정_2207072059
                post.country = form.cleaned_data['country']    # 수정_2207262128
                post.save()
...
def post_delete(request, post_id):    # 수정:2207192252
    if request.user.is_authenticated:
        post = get_object_or_404(models.Post, pk=post_id)
        if request.user == post.author:
            post.delete()
